...
def _active_sink():
    ...
    Imported lazily so ``ness_cli.tui.stream`` does not depend on ``ness_cli.tui.render`` at
    module load time (``render`` itself imports ``ness_cli.tui.stream`` for the
    ``AssistantStream`` re-export). The indirection mirrors the original
    in-method ``import time`` pattern but for the sink registry.
    """
    from ness_cli.tui.render import _sink
