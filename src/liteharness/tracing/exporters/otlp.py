...
an :class:`OTelSpan` that adapts the liteharness :class:`Span` Protocol to the
...
by :func:`liteharness.tracing.tracer.build_tracer` — see
...
# bridge between liteharness span protocol and otel span
class OTelSpan:
    """Adapts an OTel span to the liteharness :class:`Span` Protocol.

    The OTel span itself does NOT manage a contextvar; this class installs
    itself in ``_current_span`` on ``__enter__`` so downstream liteharness
...
class OTelTracer:
    ...
    def __init__(self, config: TracingConfig) -> None:
        ...
        self._tracer = provider.get_tracer("liteharness")
