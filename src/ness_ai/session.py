...
from ness_ai.graph.builder import build_graph
from ness_ai.graph.helpers import _effective_conversation
...
_active_session: ContextVar["Session | None"] = ContextVar(
    "ness_ai_active_session", default=None
)
...
class Session:
    ...
    async def finalize_reflection(self) -> None:
        ...
        if not self._cfg.options.session_end_reflection:
            return
        from ness_ai.reflection import finalize_session_reflection
    ...
    async def preview_context(self, *, mode: str | None = None) -> ContextPreview:
        ...
        **not** run the model or the compaction summarizer — compaction
        ...
        if git_flag is None:
            from ness_ai.tools.fs import is_git_repo
        ...
        from ness_ai.context.overlay import OverlayContext, wrap_system_reminder
