...
from ness_ai.context.overlay import OverlayProvider
from ness_ai.types import ApprovalHandler, QuestionHandler
...
from ness_ai.tools import BUILTIN_TOOLS, ToolRegistry
from ness_ai.utils import normalize_tool
from ness_ai.tracing.cost import CostTracker
from ness_ai.tracing.config import TracingConfig
from ness_ai.tracing.tracer import NoopTracer, Tracer, build_tracer
if TYPE_CHECKING:
...
class AgentSpec:
    ...
    **Required fields**
    ...
    ``prompt``
        L0–L2 prompt: a :class:`~ness_ai.context.layers.PromptLayers`,
        a :class:`~ness_ai.context.layers.PromptLayersConfig`, or a
    ...
    **Behaviours**
    ...
    ``options``
        ...
        approval flag, etc.
    ``overlay``
        ``None`` → default :class:`~ness_ai.context.coding_overlay.CodingOverlay`
        (plan/act, git, todos, compaction, session memory).
        Pass :class:`~ness_ai.context.coding_overlay.NoOverlay` for no L3.
    ...
    ``memory_store``
        Optional :class:`~ness_ai.memory.MemoryBackend` subclass
    ...
    **Filesystem paths**
    ...
    ``hooks``
        Optional in-memory :class:`~ness_ai.hooks.Hook` list seeded into
...
class NessAgent:
    ...
    This is the primary entry point for the Ness AI SDK. Construct
    ...
    **Minimal usage** (everything defaults to a working coding agent)::
    ...
    The default :class:`~ness_ai.context.coding_overlay.CodingOverlay`
    ...
    def __init__(self, *, model, prompt, tools: Sequence[BaseTool] | None = None, **kwargs) -> None:
        ...
        tools : iterable of BaseTool / callable / str, optional
            ...
            SDK built-in tools (:data:`~ness_ai.tools.BUILTIN_TOOLS`).
    ...
    def session(
        ...
        *,
        ...
        """Create a runnable :class:`~ness_ai.session.Session` for one thread.
        ...
        on_plan_turn, on_interrupt
            ...
            one agent never clobber each other. See :mod:`ness_ai.types`
