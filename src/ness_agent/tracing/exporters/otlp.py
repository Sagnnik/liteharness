...
an :class:`OTelSpan` that adapts the ness_agent :class:`Span` Protocol to the
...
by :func:`ness_agent.tracing.tracer.build_tracer` — see
...
from ness_agent.tracing.tracer import Span, _current_span
...
# bridge between ness_agent span protocol and otel span
class OTelSpan:
    """Adapts an OTel span to the ness_agent :class:`Span` Protocol.

    The OTel span itself does NOT manage a contextvar; this class installs
    itself in ``_current_span`` on ``__enter__`` so downstream ness_agent
...
class OTelTracer:
    ...
    def __init__(self, config: TracingConfig) -> None:
        ...
        self._tracer = provider.get_tracer("ness-agent")
