...
def _shell_start(command: str, name: str = "") -> str:
    ...
    job = {
        ...
        "exit_path": str(exit_path.relative_to(_project_root())),
        "script_path": str(script_path.relative_to(_project_root())),
    }
    jobs[job_id] = job
...
def _ensure_shell_dir() -> Path:
    shell_dir = Path(_validate_path(str(_shell_dir())))
...
def _job_script(command: str, exit_path: Path) -> str:
    ...
    return "\n".join(
        [
            "#!/usr/bin/env bash",
            f"__ness_agent_exit_file={exit_file}",
            "trap 'code=$?; printf \"%s\\n\" \"$code\" > \"$__ness_agent_exit_file\"' EXIT",
            f"bash -lc {payload}",
            "",
        ]
    )
...
def _job_path(job: dict[str, Any], key: str) -> Path:
    ...
    return Path(_validate_path(raw))
