...
- ``events_to_messages`` takes an explicit ``vision: bool`` (the heuristic
  lives in :mod:`liteharness_cli.config`) and a
  :class:`~liteharness.permissions.PermissionStore` (the ``@file`` mention
...
- ``restore_cost_from_events`` takes a target
  :class:`~liteharness.tracing.cost.CostTracker` rather than the CLI's
...
def restore_cost_from_events(events: list[dict], cost_tracker: Any) -> None:
    ...
    session's own :class:`~liteharness.tracing.cost.CostTracker` so a fresh
