...
def _store(tmp: str | Path | None = None) -> tuple[PermissionStore, tempfile.TemporaryDirectory | None]:
    if tmp is None:
        ...
        ness = root / ".ness"
        ness.mkdir()
        return PermissionStore(ness_dir=ness, project_root=root), td
    root = Path(tmp)
    ness = root / ".ness"
    ness.mkdir(exist_ok=True)
    return PermissionStore(ness_dir=ness, project_root=root), None
...
class PermissionsShellMatcherTests(unittest.TestCase):
    ...
    def test_blocks_semicolon_chain_allow_bypass(self) -> None:
        self.assertFalse(
            self.store._shell_command_matches("ls*", "ls; rm -rf /"),
        )

    def test_blocks_newline_chain(self) -> None:
        self.assertFalse(
            self.store._shell_command_matches("ls*", "ls\nrm -rf /"),
        )
    ...
    def test_blocks_output_redirect(self) -> None:
        self.assertFalse(
            self.store._shell_command_matches("echo*", 'echo "malicious" > ~/.bashrc'),
        )

    def test_blocks_append_redirect(self) -> None:
        self.assertFalse(
            self.store._shell_command_matches("cat*", "cat /etc/passwd >> ./exfil.txt"),
        )

    def test_blocks_input_redirect(self) -> None:
        self.assertFalse(
            self.store._shell_command_matches("wc*", "wc -l < input.txt"),
        )

    def test_preserves_git_status_match(self) -> None:
        self.assertTrue(
            self.store._shell_command_matches("git status*", "git status -s"),
        )

    def test_allows_quoted_semicolon(self) -> None:
        self.assertTrue(
            self.store._shell_command_matches(
                "git commit*",
    ...
    def test_shell_wildcard_rule_still_uses_safe_command_matching(self) -> None:
        ...
        with mock.patch.object(self.store, "_load", return_value=rules):
            safe_decision, safe_rule = self.store.check_with_rule(
                "shell",
                {"action": "run", "command": "pwd"},
            )
            unsafe_decision, unsafe_rule = self.store.check_with_rule(
                "shell",
                {"action": "run", "command": "pwd; sudo rm"},
            )
            read_decision, read_rule = self.store.check_with_rule(
                "shell",
...
class PermissionsSessionRuleTests(unittest.TestCase):
    ...
    def test_session_allow_matches(self) -> None:
        ...
        with mock.patch.object(self.store, "_load", return_value=empty_rules):
            self.store.persist_rule("shell:run:pytest*", "allow", scope="session")
            decision, rule = self.store.check_with_rule(
                "shell",
    ...
    def test_persistent_deny_beats_session_allow(self) -> None:
        ...
        with mock.patch.object(self.store, "_load", return_value=rules):
            self.store.persist_rule("shell:run:sudo*", "allow", scope="session")
            decision, rule = self.store.check_with_rule(
                "shell",
    ...
    def test_clear_session_rules_removes_session_allow(self) -> None:
        ...
        with mock.patch.object(self.store, "_load", return_value=empty_rules):
            self.store.persist_rule("shell:run:pytest*", "allow", scope="session")
            self.store.clear_session_rules()
            decision, _ = self.store.check_with_rule(
                "shell",
                {"action": "run", "command": "pytest tests/test_foo.py"},
            )
        self.assertEqual(decision, "ask")
    ...
    def test_persistent_allow_survives_session_clear(self) -> None:
        ...
        with mock.patch.object(self.store, "_load", return_value=rules):
            ...
            self.store.clear_session_rules()
            decision, rule = self.store.check_with_rule(
                "shell", {"action": "run", "command": "ls -la"}
            )
        self.assertEqual(decision, "allow")
...
class DefaultRuleForTests(unittest.TestCase):
    ...
    def test_git_status_generalizes_subcommand(self) -> None:
        rule = self.store.default_rule_for("shell", {"action": "run", "command": "git status -s"})
        self.assertEqual(rule, "shell:run:git status*")

    def test_missing_action_defaults_to_run(self) -> None:
        rule = self.store.default_rule_for("shell", {"command": "git status -s"})
        self.assertEqual(rule, "shell:run:git status*")
        key = self.store.pattern_key("shell", {"command": "git status --short"})
        self.assertEqual(key, "shell:run:git status --short")

    def test_pytest_generalizes_base_command(self) -> None:
        rule = self.store.default_rule_for("shell", {"action": "run", "command": "pytest tests/foo.py"})
        self.assertEqual(rule, "shell:run:pytest*")

    def test_python_module_generalizes(self) -> None:
        rule = self.store.default_rule_for("shell", {"action": "run", "command": "python -m pytest tests/"})
        self.assertEqual(rule, "shell:run:python -m pytest*")

    def test_npm_run_generalizes(self) -> None:
        rule = self.store.default_rule_for("shell", {"action": "run", "command": "npm run build"})
        self.assertEqual(rule, "shell:run:npm run build*")

    def test_web_search_allowed_by_default(self) -> None:
        with mock.patch.object(self.store, "_load", return_value=DEFAULT_RULES.copy()):
            decision, rule = self.store.check_with_rule("web_search", {"query": "python docs"})
        self.assertEqual(decision, "allow")
    ...
    def test_shell_read_allowed_by_default(self) -> None:
        with mock.patch.object(self.store, "_load", return_value=DEFAULT_RULES.copy()):
            decision, rule = self.store.check_with_rule("shell", {"action": "read", "job_id": "abc"})
        self.assertEqual(decision, "allow")
        self.assertEqual(rule, "shell:read:*")

    def test_shell_jobs_allowed_by_default(self) -> None:
        with mock.patch.object(self.store, "_load", return_value=DEFAULT_RULES.copy()):
            decision, rule = self.store.check_with_rule("shell", {"action": "jobs"})
        self.assertEqual(decision, "allow")
    ...
    def test_non_shell_uses_exact_pattern_key(self) -> None:
        ...
        self.assertEqual(
            self.store.default_rule_for("write", args),
            self.store.pattern_key("write", args),
        )
